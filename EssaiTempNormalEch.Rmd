...
La suggestion de Jacques est de repasser en marginale:

* la cible qui a été centrée réduite est déjà N(0,1).

* $Z_1$ est marginalement Student à $2g$ degrés de liberté. $g^{0.5} \times Z_1$ est Student unitaire à $2g$ degrés de liberté.
...
Z2=rgamma(N,a_Z2,b_Z2),
Z1=rnorm(N,m_Z1,(precision_Z1*Z2)^-0.5),
QNT_Z1= qnorm(pt(sqrt(g)*Z1,df=2*g))) %>% lm(QNT_Z1~theta-1, data=.)
```
Sur l'échantillon d'apprentissage, on trouve un coefficient de corrélation $\hat{\rho}=0.8803$ entre le QNT de la cible et le QNT d'une valeur tirée au hasard dans la loi conditionnelle du premier pivot sachant les deux statistiques résumées de l'ensemble échangeable.
...
pour $m=0, s=1$, c'est à dire un prior climatologique pour produire une prévision probabiliste calibrée $[\theta|QNT]$ est $N(\rho \times QNT,1-\rho^2)$
