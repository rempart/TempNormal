...
On stationnarise TOUTES les variables du tibble YX pour créer les grandeurs sur lesquelles travailler dans  le tibble d

```{r}
ete<-c(5:11)
d<-YX %>%  mutate_at(vars(matches("Run")),regul)  %>% filter(Echeance==horizon, Month %in% ete) %>% #dplyr::select(!starts_with("Run"),-Echeance) %>%
...
On peut utiliser Jags pour estimer $\mu$, $\theta$ et $h$ en disant que la marge $X$ et ses coefficients sont connus.
```{r}
...
$\hat{a}_{BMA},\hat{b}_{BMA},\hat{\sigma}_{BMA}$ sur l'échantillon de calage où les membres ont été désaisonnalisés et stationnarisés. Ils valent respectivement `r c(a_bma,b_bma, sigma_bma)` . On remarque que
