...
ete<-c(5:11)
ech=4
d<-YX %>% filter(Echeance==ech, Month %in% ete) %>% dplyr::select(!starts_with("Run"),-Echeance) %>% mutate(y=T_ecart, x=Xbar-T_ref_lis, v2=V2, jeu=ifelse(Year<2010, yes="apprentissage",no="test"))
```
...
h_hat=means123y[1]
mu_hat=means123y[2]
theta_hat=means123y[3]
```

On peut également mettre en place un algorithme EM pour réaliser cette inférence
...
for(r in 1:N) {
  ...
  Z1[r,]<-rnorm(NRM,beta*(u[r]-alpha)/(lambda^2+50*beta^2),
                lambda/sqrt(Z2[r,]*(lambda^2+50*beta^2)))
}
A<-apply(Z2,1,"mean")
...
dga<-ACPfirst %>% #filter(qui<10) %>%
 ...
 dgb<- ACPfirst %>% #filter(qui<10) %>%
  ggplot(aes(x=qui,y=PoidsMembres))+geom_bar(stat="identity")+
  labs(y="Weight in first PCA",x="Members", title="(B): Member Weights") 
 
  dgc<-ACPfirst %>% #filter(qui<10) %>%
  ggplot(aes(x=qui,y=MA))+geom_point()+geom_line()+ylim(0,11)+
  labs(y="Mean",x="Members", title="(C): Member Means") 
  
dgd<-ACPfirst %>% #filter(qui<10) %>%
  ggplot(aes(x=qui,y=SdA))+geom_point()+geom_line()+ylim(0,5)+
...
```{r}
fittest<-function(X,MTE,SDTE,N0) {
...
Hpit<-hist(GXp,seq(0,1,0.10),plot=FALSE)
chit<-sum(((Hpit[[2]]-0.1*N)^2)/(0.1*N))
TTP[2,]<-c(chit,1-pchisq(chit,9))
# test du CHI2 sur la classification PIT (10 classes) avec la p-value correspondante.
Pdiri<-rdirichlet(NR, alpha =rep(0.1+N/10,10) )
Mnom<-matrix(NA,NR,10)
for(j in 1:NR){
 Mnom[j,]<-rmultinom(1,N,Pdiri[j,]) 
}
  Postpit<-apply(((Mnom-0.1*N)^2)/(0.1*N),1,"sum")
...
sdx<-sd(X)
pX0n<-dnorm(X,mx,sdx)
LpX0n<-sum(log10(pX0n))
LpXn<-sum(log10(dnorm(X,MTE,SDTE)))
TTP[4,]<-c(LpXn,LpX0n)
...
Score<-matrix(NA,5,2)
Score[1,]<-c(Mcrps,Mlogs)
Score[2,]<-c(mean(BRJ),mean(BRWJ))
Score[3,]<-c(mean(BWoptJ),mean(BpJ))
corr<-cor(X,mpred)
